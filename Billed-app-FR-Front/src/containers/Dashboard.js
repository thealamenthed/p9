...
export default class {
  ...
  // Modification de la fonction pour gérer l'affichage des tickets
  handleShowTickets(e, bills, index) {
    const arrow = $(`#arrow-icon${index}`);
    const container = $(`#status-bills-container${index}`);
    const isOpen = arrow.attr("data-open") === "true";

    if (isOpen) {
      arrow.css({transform: "rotate(90deg)"});
      container.html("");
      arrow.attr("data-open", "false");
    } else {
      arrow.css({transform: "rotate(0deg)"});
      container.html(cards(filteredBills(bills, getStatus(index))));
      arrow.attr("data-open", "true");

      // Attacher les événements seulement sur les tickets de cette liste
